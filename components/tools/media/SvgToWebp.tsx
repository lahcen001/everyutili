...
export default function SvgToWebp() {
  ...
  const convertAll = async () => {
    ...
    for (const item of pending) {
      ...
      if (result.status === "success") {
        await saveToolResult("svg-to-webp", {
  ...
  return (
    <div className="space-y-6">
      <DropZone
        ...
        hint="Batch rasterize vector icons and illustrations to WebP — transparency preserved"
      />
      ...
      <ToolHistoryList ref={historyRef} toolSlug="svg-to-webp" />
    </div>
  );
}
