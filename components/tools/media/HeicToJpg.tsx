...
export default function HeicToJpg() {
  ...
  const handleFiles = (files: File[]) => {
    const items: QueueItem[] = files
      .filter((f) => f.type === "image/heic" || f.type === "image/heif" || /\.hei[cf]$/i.test(f.name))
  ...
  const convertAll = async () => {
    ...
    for (const item of pending) {
      ...
      if (result.status === "success") {
        await saveToolResult("heic-to-jpg", {
  ...
  const downloadAllAsZip = async () => {
    ...
    downloadBlob(blob, "converted-jpg.zip");
  };
  ...
  return (
    <div className="space-y-6">
      <DropZone
        ...
        label="Drag & drop HEIC/HEIF photos here, or click to browse"
        hint="Convert iPhone HEIC photos to universally compatible JPG"
      />
                ...
                return (
                  <Card key={item.id} className="space-y-3 p-3">
                    ...
                    <ImageComparisonSlider
                      ...
                      processedLabel={`JPG: ${formatBytes(item.resultBlob.size)} (${changeLabel})`}
                    />
                  </Card>
                );
              }
      ...
      <ToolHistoryList ref={historyRef} toolSlug="heic-to-jpg" />
    </div>
  );
}
