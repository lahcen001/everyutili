...
export default function WebpConverter() {
  ...
  const convertAll = async () => {
    ...
    for (const item of pending) {
      ...
      if (result.status === "success") {
        await saveToolResult("webp-converter", {
  ...
  return (
    <div className="space-y-6">
      <DropZone
        onFiles={handleFiles}
        accept="image/*"
        label="Drag & drop JPG, PNG, or GIF images here, or click to browse"
        hint="Batch convert to WebP — smaller files, same quality"
      />
